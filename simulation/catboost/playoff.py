...
simulation/output/wc_26_catboost.xlsx.

Flow:
  1. Load group stage predictions from wc_26_catboost.xlsx
  2. Compute group standings (Pts → W → win prob sum tiebreak)
  3. Pick the 8 best 3rd-place teams, allocate to R32 slots
  4. Re-train CatBoost on pre-WC26 data (draw_weight from config.py)
...
Run from the project root:
    python -m simulation.catboost.playoff
"""
...
XLSX_PATH  = 'simulation/output/wc_26_catboost.xlsx'
...
def predict_ko(model, team_a, team_b, profiles):
    ...
    row = pd.Series({
        ...
        'home_avg_elo_10yr':                 pa.get('avg_elo_10yr', np.nan),
