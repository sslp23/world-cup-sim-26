"""
Ensemble backtest — WC 2022 (64 matches).

Combines three models by averaging their probability outputs:
  - XGBoost           (gradient-boosted classifier)
  - CatBoost          (gradient-boosted classifier + confederation features)
  - ML-Poisson        (XGBoost regressor + Dixon-Coles score matrix)

Ensemble strategy: simple equal-weight average of the three probability
vectors. No re-training or meta-learning — purely a test of whether
diversity across modelling approaches improves over any single model.

Run from the project root:
    python -m models.ensemble.backtest
"""
...
def avg_probs(*prob_dicts):
    """Equal-weight average of N probability dicts."""
...
def run():
    ...
    for _, match in wc22.iterrows():
        ...
        p_xgb = xgb.predict_proba_row(match)
        p_cb  = cb.predict_proba_row(match)
        p_mlp = mlp.predict_proba_row(match)

        p_ens = avg_probs(p_xgb, p_cb, p_mlp)
        predicted = max(p_ens, key=p_ens.get)
        ...
        rows.append({
            ...
            'p_home_win': round(p_ens['home_win'], 3),
            'p_draw':     round(p_ens['draw'], 3),
            'p_away_win': round(p_ens['away_win'], 3),
            'p_correct':  round(p_ens[outcome], 3),
            'log_loss':   round(log_loss_single(p_ens, outcome), 4),
            'rps':        round(rps(p_ens, outcome), 4),
        })
    ...
    out_path = "models/ensemble/wc2022_backtest_results.csv"
