"""
Backtest CatBoost model against FIFA World Cup 2022.
...
def run():
    ...
    model = CatBoostPredictor(draw_weight=0.75)
    model.fit(train_df)

    print("\nFeature importances:")
    fi = model.feature_importance()
    for _, row in fi.iterrows():
        bar = '#' * int(row['importance'] * 15)
        print(f"  {row['feature']:<26}  {row['importance']:>5.2f}%  {bar}")
    ...
    out_path = "models/catboost/wc2022_backtest_results.csv"
