"""
Backtest ML-Poisson model against FIFA World Cup 2022.

Training data : all matches before 2022-11-20
...
def run():
    ...
    print("\nSample lambdas on first 5 WC matches:")
    for _, match in wc22.head(5).iterrows():
        lam, mu = model.predict_lambdas(match)
        print(f"  {match['home_team']:<22} vs {match['away_team']:<22}  lambda_home={lam:.3f}  lambda_away={mu:.3f}")
    ...
    out_path = "models/ml_poisson/wc2022_backtest_results.csv"
