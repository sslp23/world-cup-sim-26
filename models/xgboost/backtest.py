"""
Backtest XGBoost model against FIFA World Cup 2022.

Training data : all matches in ranked_database_with_features.csv before 2022-11-20
...
Metrics reported per match and in aggregate:
  ...
  - Log-Loss  : -mean(log p_correct)
  - RPS       : Ranked Probability Score (ordered home_win / draw / away_win)
"""
...
def run():
    ...
    # ── Fit model ─────────────────────────────────────────────────────────────
    model = XGBoostPredictor(draw_weight=0.65)
    model.fit(train_df)
    ...
    # ── Save results ──────────────────────────────────────────────────────────
    out_path = "models/xgboost/wc2022_backtest_results.csv"
